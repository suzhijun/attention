...
from rpn_msr.proposal_layer_detection import proposal_layer as proposal_layer_py
...
class RPN(nn.Module):
	...
	anchor_scales_normal = [2, 4, 8, 16, 32]
	anchor_ratios_normal = [0.25, 0.5, 1, 2, 4]
	anchor_scales_normal_relationship = [4, 8, 16, 32, 64]
	...
	def __init__(self, use_kmeans_anchors=False):
		...
		self.score_conv_region = Conv2d(512, self.anchor_num_relationship*2, 1, relu=False, same_padding=False)
		self.bbox_conv_region = Conv2d(512, self.anchor_num_relationship*4, 1, relu=False, same_padding=False)
	...
	@property
	def loss(self):
		return self.cross_entropy_relationship + self.loss_box_relationship*0.5
	...
	def forward(self, im_data, im_info, gt_objects=None, gt_box_relationship=None, dontcare_areas=None):
		...
		rpn_conv1_relationship = self.conv1_region(features)
		rpn_cls_score_relationship = self.score_conv_region(rpn_conv1_relationship)
		...
		rpn_bbox_pred_relationship = self.bbox_conv_region(rpn_conv1_relationship)
	...
	def anchor_target_layer(rpn_cls_score, gt_boxes, dontcare_areas, im_info, _feat_stride, anchor_scales,
							anchor_rotios, is_relationship=False):
		"""
		...
		rpn_labels, rpn_bbox_targets, rpn_bbox_inside_weights, rpn_bbox_outside_weights = \
			anchor_target_layer_py(rpn_cls_score, gt_boxes, dontcare_areas, im_info, _feat_stride, anchor_scales,
								   anchor_rotios, is_relationship=is_relationship)
...
class FasterRCNN(nn.Module):
	...
	def loss(self):
		...
		return 5*self.cross_entropy + self.loss_box
