...
class Hierarchical_Message_Passing_Structure_base(nn.Module):
	def __init__(self, nhidden, n_object_cats, n_predicate_cats, dropout=False, gate_width=128,
		...
		self.gate_sub2pred = Message_Passing_Unit(nhidden, gate_width)
		self.gate_obj2pred = Message_Passing_Unit(nhidden, gate_width)
		self.gate_pred2sub = Message_Passing_Unit(nhidden, gate_width)
		self.gate_pred2obj = Message_Passing_Unit(nhidden, gate_width)

		self.GRU_object = Gated_Recurrent_Unit(nhidden, dropout)  # nn.GRUCell(fea_size, fea_size) #
		self.GRU_phrase = Gated_Recurrent_Unit(nhidden, dropout)
	...
	def prepare_message(self, target_features, source_features, select_mat, gate_module):
		...
		for f_id in range(target_features.size()[0]):
			if len(np.where(select_mat[f_id, :] > 0)[0]) > 0:
				...
				temp = Variable(torch.zeros(target_features.size()[1:]), requires_grad=True).type(
